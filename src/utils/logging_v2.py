...
def get_logger(name: str = None, log_dir: str = None) -> logging.Logger:
    ...
    # Only set handlers if they haven't been set before
    if not logger.handlers:
        ...
        if log_dir is not None:
            ...
            file_handler = logging.FileHandler(
                os.path.join(log_dir, f"{name if name else 'train'}.txt"),
                mode="w"
            )
